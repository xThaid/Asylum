...
ENERGY_PRODUCTION_OFFSET += 118500 # 14.02.2018
ENERGY_PRODUCTION_OFFSET += 529042130 # 11.10.2020


def get_flara_data():
    try:
        res = requests.get(config['SUBSYSTEMS']['flara_url'] + "list_en.html", timeout=1).text
        if res != "<tr class='msgfail'><td>Devices not found.</td></tr>":
            res = requests.get(config['SUBSYSTEMS']['flara_url'] + "FF00000080087B2E00010102000001EF/data_en.html", timeout=1).text
            return {
...
def get_emeter_data():
    try:
        emeter_reading = requests.get(config['SUBSYSTEMS']['emeter_url'], timeout=1).json()
